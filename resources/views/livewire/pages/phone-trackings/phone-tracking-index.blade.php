        ...
        <div class="w-full mb-1">
            ...
            <div class="items-center hidden mb-3 sm:flex sm:divide-x sm:divide-gray-100 sm:mb-0 dark:divide-gray-700">
                <form class="lg:pr-3" action="#" method="GET">
                    <label for="search-company" class="sr-only">Search</label>
                    <div class="relative mt-1 lg:w-64 xl:w-96">
                        <select name="search-company" id="search-company" wire:model="selectedCompanyID" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
            ...
            @if(count($phoneNumbers['data']) > 0)
                <div class="flex items-center ml-auto space-x-2 sm:space-x-3">
                    <button type="button" wire:click="createPhoneNum" class="inline-flex items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-primary-300 sm:w-auto dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-primary-800">
                        ...
                        Add Phone Number {{$selectedCompanyID}}
                    </button>
                    <a href="{{ route('call-histories', ['company'=> $selectedCompany->id]) }}" class="inline-flex items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:ring-primary-300 sm:w-auto dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-700">
                ...
                <x-organisms.table>
                    <x-molecules.tables.thead>
                        <tr>
                            ...
                            <x-atoms.tables.th>Capabilities</x-atoms.tables.th>
                    ...
                    <x-molecules.tables.tbody>
                        @foreach ($phoneNumbers['data'] as $phoneNumber)
                            <tr>
                                ...
                                <x-atoms.tables.td>
                                    @foreach ( $phoneNumber['capabilities'] as $capabilities )
                                        <span class="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-green-400 border border-green-400">{{ strtoupper($capabilities) }}</span>
                                    @endforeach
                                </x-atoms.tables.td>
                                <x-atoms.tables.td>
